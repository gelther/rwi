...
?>
> <span><?php
printf( __rw( 'visibility_show-on-every' ), $type );
?>
</span>
				</div>
				<div class="rw-ui-img-radio rw-ui-hor<?php
if ( $visibility->selected == 1 ) {
    echo  ' rw-selected' ;
}
?>
...
>
					<span><?php
printf( __rw( 'visibility_show-on-every-exclude' ), $type, $types );
?>
 </span>
					<input type="text" name="rw_visibility_exclude"
					       value="<?php
echo  implode_or_empty( $visibility->exclude ) ;
?>
"/>
				</div>
				<div class="rw-ui-img-radio rw-ui-hor<?php
